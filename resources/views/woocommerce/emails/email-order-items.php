...
foreach ( $items as $item_id => $item ) :
	...
	<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
				...
				<table class="order-item-data" role="presentation">
					<tr>
						...
						if ( $show_image ) {
							 ...
							 * @since 2.1.0
							 */
							echo '<td class="email-order-item-image" style="width: 64px; min-width: 64px; padding-right: 16px; vertical-align: middle;">' . wp_kses_post( apply_filters( 'woocommerce_order_item_thumbnail', $image, $item ) ) . '</td>';
						}
						?>
						<td>
							<?php
		...
		<td class="td font-family text-align-<?php echo esc_attr( $price_text_align ); ?>" style="vertical-align:middle;">
			<?php
			$is_bundled = $item->get_meta( '_bundled_by' );
			if ( ! $is_bundled ) :
