...
<li class="product product-card {{ implode(' ', wc_get_product_class('', $product_id)) }}">
  ...
  <div class="product_content">
    ...
    {{-- Countdown reducere — doar pe cardurile din slider-ele home
         (flag de context) și doar pe pachetele cu reducerea BF activă. --}}
    @php
      $mn_card_cd = ! empty($GLOBALS['mn_card_countdown'])
          && function_exists('App\\bf_bundle_sale_price')
          && \App\bf_bundle_sale_price($product) !== null;
      $mn_cd_deadline = $mn_card_cd ? \App\bf_deadline_timestamp() * 1000 : 0;
    @endphp
    @if ($mn_card_cd && $mn_cd_deadline > 0)
      @include('partials.bf-countdown-card', [
        'deadlineMs' => $mn_cd_deadline,
        'percent' => $discount > 0 ? $discount : \App\BF_PERCENT,
      ])
    @endif
