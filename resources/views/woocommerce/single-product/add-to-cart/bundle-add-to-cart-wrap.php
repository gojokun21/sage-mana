...
?>
<div class="cart bundle_data bundle_data_<?php echo esc_attr( $product_id ); ?>"
	data-bundle_form_data="<?php echo wc_esc_json( wp_json_encode( $bundle_form_data ) ); ?>"
	data-bundle_id="<?php echo esc_attr( $product_id ); ?>">
	<?php

	if ( $is_purchasable ) {

		/** WC Core action. */
		do_action( 'woocommerce_before_add_to_cart_button' ); // phpcs:ignore WooCommerce.Commenting.CommentHooks.MissingSinceComment
		...
		/** WC Core action. */
		do_action( 'woocommerce_after_add_to_cart_button' ); // phpcs:ignore WooCommerce.Commenting.CommentHooks.MissingSinceComment
